...
m) max_lines=${OPTARG};; # The maximum number of lines per each temporary fastq file in order to avoid memory errors
esac
done
...
then
	echo "Using max_lines to split the fastq files ... "
	if (( $max_lines % 4 )) ; then
