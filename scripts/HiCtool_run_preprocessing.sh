...
c) chunk_size=${OPTARG};; # The number of lines per each temporary fastq file in order to avoid memory errors. Each temporary file is processed by a separate processor if multiple threads are used. 1000 lines means 250 reads.
esac
done
