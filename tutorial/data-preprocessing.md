...
- ``-m``: if your data are very big, you may encounter a memory error when the fastq files are loaded for pre-truncated and when downstream when the paired reads between the two mapped files are selected. Thus, you may use this parameter in order to split the two fastq files into several temporary files with ``-m`` lines each, that are pre-truncated separately. The temporary files will be processed with multi-threads if you are using multiple processors (``-p``). Therefore, setting ``-m`` may help to speed up the pre-truncation process. In addition, setting ``-m`` lets the program working with smaller temporary files at the pairing step as well.
